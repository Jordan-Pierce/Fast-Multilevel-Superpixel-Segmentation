...
def get_scores(ground_truth_files, prediction_files, labels):
    ...
    for _ in range(num_files):
        ...
        for c in range(num_classes):
            ...
            predicted[c] = predicted[c] + sum(
                sum((ground_truth >= 0) & (ground_truth < num_classes) & (prediction == c)))
    ...
    RA = pd.DataFrame(
        list(zip(labels, np.around(actual / np.sum(actual), 4), np.around(predicted / np.sum(predicted), 4))),
    ...
    print("\nOverall Accuracy: ",
          np.sum(np.array(flat_ground_truth) == np.array(flat_prediction)) / len(flat_ground_truth))
